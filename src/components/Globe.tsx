import { Globe } from "@/components/ui/globe";

export default function GlobeCard() {
  return (
    <div className="bg-background relative flex size-full max-w-full min-w-0 items-center justify-center overflow-hidden rounded-lg border md:px-40 pt-8 pb-40 md:pb-60">
      <span className="pointer-events-none bg-linear-to-b from-black to-gray-300/80 bg-clip-text text-center text-8xl leading-none font-semibold whitespace-pre-wrap text-transparent dark:from-white dark:to-slate-900/10">
        Our Reach
      </span>
      <Globe className="top-52" />
      <div className="pointer-events-none absolute inset-0 h-full bg-[radial-gradient(circle_at_50%_200%,rgba(0,0,0,0.2),rgba(255,255,255,0))]" />
    </div>
  );
}
